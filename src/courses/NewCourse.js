...
/**
 * A page that allows the user to create a new course.
 */
export default function NewCourse({ loadDashboard }) {
    ...
    function handleSubmit(event) {
        ...
        if (validateFields()) {
            createCourse(formData, abortController.signal)
                .then(loadDashboard)
    ...
    function validateFields() {
        ...
        if (formData.course_name === "" || formData.capacity === "") {
            foundError = { message: "Please fill out all fields." };
        }
        else if (formData.course_name.length < 2) {
            foundError = { message: "Course name must be at least 2 characters." };
        }
